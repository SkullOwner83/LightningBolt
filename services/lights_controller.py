import time
import asyncio
from bleak import BleakClient
from models.device import Device

class LightsController:
    RECONNECT_TIMEOUT = 10.0
    RECONNECT_COOLDOWN = 30.0

    def __init__(self, device: Device):
        self.device = device
        self.client: BleakClient | None = None
        self._reconnecting: bool = False
        self._last_reconnect: float = 0.0
    
    @property
    def is_connected(self) -> bool:
        return self.client is not None and self.client.is_connected

    async def connect(self) -> None:
        if self.is_connected or self._reconnecting: return
        self._reconnecting = True

        try:
            self.client = BleakClient(self.device.address, timeout=self.RECONNECT_TIMEOUT)
            await self.client.connect()
            print(f"Connected to device '{self.device.name}'")
        finally:
            self._reconnecting = False

    async def disconnect(self) -> None:
        if not self.is_connected: return
        await self.client.disconnect()
        print(f"Device '{self.device.name}' is disconnected")

    async def _reconnect(self) -> None:
        self._last_reconnect = time.monotonic()
        self._reconnecting = True
        
        try:
            self.client = BleakClient(self.device.address, timeout=self.RECONNECT_TIMEOUT)
            await self.client.connect()
            print(f"Connected to device '{self.device.name}'")
        except Exception as ex:
            print(f"Reconnect failed for '{self.device.name}': {ex}")
            self.client = None
        finally:
            self._reconnecting = False

    async def set_color(self, r:int, g:int, b: int) -> None:
        if not self.is_connected:
            now = time.monotonic()

            if not self._reconnecting and (now - self._last_reconnect) >= self.RECONNECT_COOLDOWN:
                asyncio.create_task(self._reconnect())

            return

        command = bytes([0x7e, 0x07, 0x05, 0x03, r, g, b, 0x0a, 0xef])
        await self.client.write_gatt_char(self.device.char_uuid, command, response=False)